...
export const getConversations = socket => dispatch => {
  axios
    .get(`${api}/api/messages/get_conversations`)
    .then(res => {
...
export const getConversation = id => dispatch => {
  ...
  axios
    .get(getConversation + otherUserId)
    .then(res => {
...
export const fetchConversation = ({ data }) => {
  if (data.conversationId) {
    const conversation = {
      ...
      title: data.title[0].name,
      avatar: data.title[0].avatar,
      username: data.title[0].username
    };
    return {
...
export const newConversation = (socket, recipientId, content) => dispatch => {
  axios
    .post(`${api}/api/messages/new_conversation/${recipientId}`, {
...
export const confirmVideoConversation = (
  socket,
  conversationId
) => dispatch => {
...
export const declineVideoConversation = (
  socket,
  conversationId
) => dispatch => {
...
export const stopVideoConversation = (
  socket,
  conversationId,
  history
) => dispatch => {
